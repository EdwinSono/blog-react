...
function BlogPost() {
  ...
  const auth = useAuth();
  const user = useUser();
  const { blogs, remove: removeBlog } : any= useBlog();

  const blogpost = blogs.find((post: { slug: string | undefined; }) => post.slug === slug);
  ...
  return (
    <>
      ...
      {blogpost.comments.length > 0 ? (
        <ul>
          {blogpost.comments.map((comment: { author: any; }, idx: number) => (
            <Comment
              comment={comment}
